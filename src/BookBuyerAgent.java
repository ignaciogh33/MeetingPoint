	  ...
	  public void action() {
	    ...
	    case 1:
		  ...
		  if (System.currentTimeMillis() - startTime > timeout) {
			//timeout reached, continue execution even if not all agents responded
			System.out.println(getAID().getLocalName() + ": " + targetBookTitle + " purchase has failed. Timeout reached.");
			step = 2;
		   } 
	      ...
	      if (reply != null) {
	        ...
	        if (reply.getPerformative() == ACLMessage.PROPOSE) {
	          //proposal received
	          String bookId = reply.getContent();
			  targetBookId = bookId;
	      ...
	      break;
	    case 2:
			if (((BookBuyerAgent) myAgent).getBudget() < bestPrice) {
				System.out.println(getAID().getLocalName() + ": " + targetBookTitle + " is too expensive for me.");
	    ...
	    case 3:      
	      ...
	      if (reply != null) {
	        if (reply.getPerformative() == ACLMessage.INFORM) {
	          //purchase succeeded
	          System.out.println(getAID().getLocalName() + ": " + targetBookTitle + " purchased for " + bestPrice + " from " + reply.getSender().getLocalName() + " Budget: " + (((BookBuyerAgent) myAgent).getBudget() - bestPrice));
	          ((BookBuyerAgent) myAgent).budget -= bestPrice;
